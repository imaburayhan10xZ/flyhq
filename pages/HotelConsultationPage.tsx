...
import { getHotelOffers, saveConsultation } from '../services/firebaseService';
import { ArrowLeft, Send, BedDouble, Loader2 } from 'lucide-react';

const HotelConsultationPage: React.FC = () => {
    ...
    if (!hotel) {
         return (
            <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center p-10 text-center pb-20">
                ...
                <button onClick={() => navigate('/hotels')} className="text-primary hover:underline font-medium">Return to Hotels</button>
            </div>
        );
    }
    ...
    return (
        <div className="min-h-screen bg-slate-50 py-16">
            <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
                <button onClick={() => navigate(`/hotels`)} className="flex items-center text-slate-500 hover:text-slate-900 mb-8 transition font-medium">
                    <ArrowLeft className="w-5 h-5 mr-2" /> Back to Hotels
                </button>

                <div className="bg-white rounded-3xl shadow-xl border border-slate-100 overflow-hidden">
                    <div className="bg-blue-600 text-white p-8 md:p-10 text-center relative overflow-hidden">
                         <div className="absolute top-0 left-0 w-full h-full opacity-10 bg-[url('https://www.transparenttextures.com/patterns/cubes.png')] pointer-events-none"></div>
                        <BedDouble className="w-12 h-12 mx-auto mb-4 text-blue-100 relative z-10" />
                    ...
                    <div className="p-8 md:p-10">
                        <form onSubmit={handleSubmit} className="space-y-6">
                            ...
                            <div>
                                <label className="block text-sm font-bold text-slate-700 mb-2">Check-in / Check-out Dates & Notes</label>
                                <textarea 
                                    rows={4} 
                                    maxLength={1000}
                                    ...
                                    className="w-full bg-slate-50 border border-slate-200 text-slate-900 rounded-xl focus:ring-blue-500 focus:border-blue-500 block p-3.5 outline-none transition" 
                                    placeholder="Please provide your expected check-in/out dates, guest count, and any other requests..." 
                                ></textarea>
                            </div>
                            ...
                            <p className="text-xs text-center text-slate-400 font-medium">Our accommodation expert will contact you shortly to finalize your booking.</p>
                        </form>
                    </div>
                </div>
            </div>
...
export default HotelConsultationPage;
