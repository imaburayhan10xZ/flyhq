...
import { getDestinations, getBlogPosts, getBanners } from '../services/firebaseService';
...
const HomePage: React.FC<HomePageProps> = ({ onSearch, onHotelSearch }) => {
  ...
  useEffect(() => {
    ...
    getBanners().then(data => {
        if (data && data.length > 0) {
            setBanners(data.filter((b:any) => b.isActive).sort((a:any, b:any) => a.order - b.order));
        }
    }).catch(err => console.error(err));
  }, []);
  ...
  return (
    <div className="relative bg-white">
      ...
      <motion.div 
        ...
        className="max-w-7xl mx-auto px-4 -mt-40 relative z-20"
      >
              ...
              <div className="relative w-full overflow-hidden group pb-4">
                  ...
                  >
                      {banners.map((b: any, idx: number) => (
                          ...
                          >
                              <div className="w-full h-[180px] sm:h-[200px] md:h-[240px] lg:h-[280px] xl:h-[300px] rounded-xl sm:rounded-2xl overflow-hidden shadow-lg border border-slate-100 bg-slate-50">
